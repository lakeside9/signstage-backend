...
/**
 * 조직×플랜 세밀 할인 오버라이드 관리 API — 행 하나가 기간 하나(다중 버전, 안 B). signstage-docs
 ...
 * 옛 선택옵션/용량 추가구매 오버라이드 엔드포인트는 2단계 전환으로 제거됐다(signstage-docs
...
public class PlatformAdminOrganizationDiscountController {
    ...
    @Operation(
            ...
            description = "이 조직의 플랜별 오버라이드를, 각 플랜의 모든 기간(과거/현재/예정)과 함께 한 번에 반환한다."
    )
    @GetMapping
