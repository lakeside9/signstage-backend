...
public class CeremonyController {
    ...
    @Operation(summary = "행사 목록 조회", description = "OPERATOR는 본인이 배정된 행사만 조회된다.")
    @GetMapping
    public ApiResponse<List<CeremonyDto.Response.CeremonySummary>> findCeremonies(
            @AuthenticationPrincipal CurrentUser currentUser,
            @PathVariable Long organizationId
    ) {
        List<CeremonyDto.Response.CeremonySummary> response =
                ceremonyService.findCeremonies(organizationId, currentUser.userId());
        return ApiResponse.success(response, traceIdProvider.getTraceId());
    }
