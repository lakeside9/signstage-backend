...
public interface CeremonyUnitProductPurchaseRepositoryCustom {
    ...
    /**
     * 플랫폼 관리자 구매 이력 조회용. status/organizationId/ceremonyId 전부 선택 필터다(null이면
     * 그 조건을 걸지 않는다) — 기존 "승인 큐"(status=PENDING 기본)와 신규 "행사 이력" 화면
     * (ceremonyId로 좁힘)이 같은 조회를 공유한다(signstage-docs
     * business/unit-product-purchase-self-checkout-review.md 8.6절 결정, 2026-09-11).
     */
    Page<CeremonyUnitProductPurchase> search(PurchaseStatus status, Long organizationId, Long ceremonyId, Pageable pageable);
}
