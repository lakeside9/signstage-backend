...
/**
 * 조직×품목 할인 오버라이드 조직 횡단 목록 API — 조직 상세를 거치지 않는 별도 목록 화면 전용
 ...
 * 그대로 재사용한다(같은 문서 6장 결정 #2) — 이 컨트롤러는 읽기 전용 목록 3개만 새로 추가한다.
 * 조회 전용이라 등급 검사 없이 PLATFORM_SUPPORT 이상이면 누구나 호출할 수 있다.
 */
@Tag(name = "PlatformAdmin", description = "플랫폼 관리자 조직×품목 할인 오버라이드 횡단 목록 API")
@RestController
...
public class PlatformAdminOrganizationDiscountListController {
    ...
    @Operation(summary = "조직×선택옵션 할인 오버라이드 조직 횡단 목록", description = "organizationId는 선택 필터다. 생략하면 전체 조직을 반환한다.")
    @GetMapping("/api/platform-admin/billing-discounts/optional-features")
    public ApiResponse<PageResponse<OrganizationDiscountDto.Response.OptionalFeatureDiscountSummary>> findOptionalFeatureDiscounts(
            @RequestParam(required = false) Long organizationId,
            @PageableDefault(size = 20) Pageable pageable
    ) {
        Page<OrganizationDiscountDto.Response.OptionalFeatureDiscountSummary> response =
                organizationDiscountService.findOptionalFeatureDiscountsAcrossOrganizations(organizationId, pageable);
        return ApiResponse.success(PageResponse.from(response), traceIdProvider.getTraceId());
    }

    @Operation(summary = "조직×용량 추가구매 할인 오버라이드 조직 횡단 목록", description = "organizationId는 선택 필터다. 생략하면 전체 조직을 반환한다.")
    @GetMapping("/api/platform-admin/billing-discounts/capacity-addons")
    public ApiResponse<PageResponse<OrganizationDiscountDto.Response.CapacityAddOnDiscountSummary>> findCapacityAddOnDiscounts(
            @RequestParam(required = false) Long organizationId,
            @PageableDefault(size = 20) Pageable pageable
    ) {
        Page<OrganizationDiscountDto.Response.CapacityAddOnDiscountSummary> response =
                organizationDiscountService.findCapacityAddOnDiscountsAcrossOrganizations(organizationId, pageable);
        return ApiResponse.success(PageResponse.from(response), traceIdProvider.getTraceId());
    }
}
