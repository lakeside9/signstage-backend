 ...
 * 목적·같은 스냅샷 원칙·같은 다중 버전(기간) 모델이다 — 이 조직이 이 상품을 구매할 때
 * ({@code CeremonyService#purchaseCapacity}) 그 순간
 * {@code CeremonyCapacityPurchase.purchasedDiscountType/Value}로 스냅샷되므로, 나중에 여기
 * 값을 바꿔도 이미 구매된 건에는 영향을 주지 않는다.
 */
@Entity
@Table(
        name = "organization_capacity_addon_discounts",
        uniqueConstraints = @UniqueConstraint(
                name = "uq_ocad_organization_addon_period",
                columnNames = {"organization_id", "capacity_addon_id", "effective_from"}
        )
)
@Getter
